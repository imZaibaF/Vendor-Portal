...

{
...

	{
		...
		/*		DriverManager
		  ...
		  con =
				  DriverManager.getConnection("jdbc:mysql://localhost:3306/rwf","root","Zai21@quen"); 
	...
	public static String  authaUser(String usr, String pass) {
		...
		try {
			...
			stmt = abrwf.conn().createStatement();
			rs = stmt.executeQuery("select  uname,pass from login where uname='"+usr+"' and pass='"+pass+"'");

			if (rs.next())
				return "Authentication Suceessful!";
			else
...
public static String  insertUser(String usr, String pass) {
	...
	try {
		...
		//ConDB.ConEstablish(); 
		//ConDB.viewRecs();
System.out.println(abrwf.authaUser("rr", "oo"));
System.out.println(abrwf.insertUser("rr", "oo"));
